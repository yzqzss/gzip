...
func TestOnlyDecompress(t *testing.T) {
	buf := &bytes.Buffer{}
	gz, _ := gzip.NewWriterLevel(buf, gzip.DefaultCompression)
	if _, err := gz.Write([]byte(testResponse)); err != nil {
		gz.Close()
	...
	router := gin.New()
	router.Use(Gzip(NoCompression, WithOnlyDecompress(true), WithDecompressFn(DefaultDecompressHandle)))
	router.POST("/", func(c *gin.Context) {
		if v := c.Request.Header.Get("Content-Encoding"); v != "" {
			t.Errorf("unexpected `Content-Encoding`: %s header", v)
		}
...
func TestGzipWithOnlyDecompress(t *testing.T) {
	buf := &bytes.Buffer{}
	gz, _ := gzip.NewWriterLevel(buf, gzip.DefaultCompression)
	if _, err := gz.Write([]byte(testResponse)); err != nil {
		gz.Close()
	...
	w := httptest.NewRecorder()
	r := gin.New()
	r.Use(Gzip(NoCompression, WithOnlyDecompress(true), WithDecompressFn(DefaultDecompressHandle)))
	r.POST("/", func(c *gin.Context) {
		assert.Equal(t, c.Request.Header.Get("Content-Encoding"), "")
		assert.Equal(t, c.Request.Header.Get("Content-Length"), "")
		body, _ := c.GetRawData()
		body_str := string(body)
		assert.Equal(t, testResponse, body_str)
		c.String(200, testResponse)
	})
	r.ServeHTTP(w, req)
